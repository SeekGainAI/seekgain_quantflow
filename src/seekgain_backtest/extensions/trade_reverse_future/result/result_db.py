...
class ResultDb(object):
    ...
    def save_result_to_db(self, last_strategy_profit, ar, last_standard_profit, sr, alpha, beta, sharpe, vol,
        ...
        #                       {'$set': update_dict})
        self.mongo_client.mongo_update_one(config["MONGO_DB"],collection_name="seekgain_back_test",query={"_id": ObjectId(run_id)},update={"$set": update_dict},upsert=True)
    ...
    def save_account(self, account_list):
        ...
        self.mongo_client.mongo_insert_many(db_name=config["MONGO_DB"],collection_name="seekgain_backtest_account",documents=account_list)
    ...
    def save_position(self, all_position_list):
        ...
        self.mongo_client.mongo_insert_many(db_name=config["MONGO_DB"],collection_name="seekgain_backtest_position",documents=all_position_list)

    def save_trade(self, all_trade_list):
        # trade_col = self.mongo_client.xb_backtest_trade
        # trade_col.insert_many(all_trade_list)
        self.mongo_client.mongo_insert_many(db_name=config["MONGO_DB"], collection_name="seekgain_backtest_trade",
    ...
    def save_profit(self, all_profit_data):
        ...
        try:
            ...
            if validated_documents:
                # print("开始MongoDB插入操作")
                self.mongo_client.mongo_insert_many(db_name=config["MONGO_DB"], collection_name="seekgain_backtest_profit",
    ...
    def save_draw(self, chart_data):
        ...
        self.mongo_client.mongo_update_one(config["MONGO_DB"], collection_name="seekgain_custom_chart",
