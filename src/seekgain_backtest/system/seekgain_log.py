        ...
        while True:
            try:
                ...
                if insert_content['level'] == -1:
                    ...
                    # 收到结束信号
                    if len(log_list) > 0:
                        mongo_client.mongo_insert_many(config["MONGO_DB"], collection_name="seekgain_user_strategy_log", documents=log_list)
                ...
                if len(log_list) > 50:
                    ...
                    log_list = list()
                    mongo_client.mongo_insert_many(config["MONGO_DB"], collection_name="seekgain_user_strategy_log", documents=insert_log_list)
