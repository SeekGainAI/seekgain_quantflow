...
def initialize(context):
    ...
    context.factor_name = "dQc47fb1b"
    context.start_date="20241001"
    context.end_date="20241231"
    SRLogger.info("factor_name:" + context.factor_name)

    context.adjustment_cycle=1
    context.group_number=5
    SRLogger.info("adjustment_cycle:" + str(context.adjustment_cycle) + "\ngroup_number:" + str(context.group_number))

    # 自定义累计交易日，用于换仓判断
    context.trade_days = 0

    # 获取因子值，进行shift操作对齐数据，由于交易当天需要获取前一天的因子值
    context.df_factor = (seekgain_data.get_factor_by_name(factor_name=context.factor_name, start_date=context.start_date,
...
def before_trading(context):
    ...
    # 调仓
        ...
        # 获取当前持仓
        df_current= context.stock_account_dict['15032863'].positions.to_dataframe()
...
def after_trading(context):
    ...
    print(f"交易后: day={context.now},total_value ={context.stock_account_dict['15032863'].total_value}")
...
def get_target_positions(context):
    ...
    today_df_factor = context.df_factor[context.df_factor["date"] == today].sort_values(context.factor_name, ascending=False)
    # 获取前几行的 "symbol" 列，并转换为列表
    today_trade_symbol = pd.DataFrame(
        today_df_factor.head(len(today_df_factor) // context.group_number)["symbol"].tolist(),columns=["symbol"])
