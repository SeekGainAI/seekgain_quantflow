...
import time
import traceback
from seekgain_backtest.backtest_common.data.quotation.back_test.base_bar_data_source import BaseBarDataSource
from seekgain_backtest.backtest_common.model.quotation.daily_quotation_data import DailyQuotationData
from seekgain_backtest.backtest_common.model.quotation.bar_quotation_data import BarQuotationData
...
import pymongo
from seekgain_backtest.util.time.time_util import TimeUtil
...
class BarDataSource(BaseBarDataSource):
    ...
    def get_future_daily_bar(self, symbol, date):
        ...
        # bar_dict = self.quotation_mongo_db.mongo_find_one(db_name="seekgain",collection_name=collection,query={"trade_date": int(date), "symbol": symbol})
        bar_dict = self.quotation_mongo_db.mongo_find_one(db_name="seekgain",collection_name=collection,query={"date": str(date), "symbol": symbol.split(".")[0]})
        bar = DailyQuotationData()
        if bar_dict:
            bar.__dict__ = bar_dict
            # print(bar_dict)
    ...
    def init_future_daily_quotation(self, symbol_list, trade_date):
        ...
        bar_cur = self.quotation_mongo_db.mongo_find(db_name="seekgain",collection_name="future_1d_market",query={"date": str(trade_date), "symbol": {'$in': processed_symbol_list}})
