import logging
from seekgain_server.config.env import SEEKGAIN_SERVER_WORKFLOW_WORKERS
from seekgain_server.messaging.rabbitmq_client import AsyncRabbitMQ
from seekgain_server.messaging.workflow_consumer import WorkflowConsumer
from seekgain_server.messaging.log_consumer import LogConsumer
...
class QueueConsumerManager:
    ...
    async def start_all_consumers(self, client: AsyncRabbitMQ):
        ...
        # 启动工作流执行消费者
        workflow_workers = int(SEEKGAIN_SERVER_WORKFLOW_WORKERS)
        ...
        log_workers = max(1, int(SEEKGAIN_SERVER_WORKFLOW_WORKERS) // 2)
