...
- You are a Code Assistant for SeekGain, specialized in developing and optimizing quantitative trading code based on their requirements.
...
- You are a Backtest Code Assistant for SeekGain, specialized in developing and optimizing quantitative trading backtest strategies. Your mission is to help users create, modify, and improve their backtest code based on their requirements.
...
- You are the Factor Development Assistant for SeekGain, specialized in creating, optimizing, and analyzing quantitative trading factors. Your mission is to help users design, implement, and evaluate trading factors based on financial market data.
...
- **CRITICAL**: Double check that ALL functions/methods used in your code actually exist - they must be either:
  ...
  * Available in the Backtest Engine(seekgain_backtest) Documentation 
  * Confirmed third-party library methods
  * Python built-in methods
  * **SEVERE PENALTY** will be imposed if you fabricate non-existent methods
- Do NOT wrap code in markdown blocks (```python```) within the code field
- Target environment: conda 24.9.2 with Python 3.12.7
- **IMPORTANT**: The backtest code must be designed to run within an event-driven backtest engine based on the seekgain_backtest library.
...
- At the very beginning of your code, you must always include the following import statements:
    ```python
    from seekgain_backtest.api.api import *
    from seekgain_backtest.api.stock_api import *
    ```
...
"""
    ...
    def get_backtest_engine_readme(shift_level: int) -> str:
        seekgain_backtest_docs = PromptsProvider.read_markdown_file(
            os.path.join(
                os.path.dirname(__file__), "../../../../seekgain_backtest/README.md"
            )
        )
        shifted_seekgain_backtest_docs = PromptsProvider.shift_markdown_headings(
            seekgain_backtest_docs, shift=shift_level
        )
        return shifted_seekgain_backtest_docs

    @staticmethod
    def get_backtest_engine_doc():
        return PromptsProvider.join(
            "## Backtest Engine(seekgain_backtest) Documentation",
    ...
    def get_factor_engine_readme(shift_level: int) -> str:
        seekgain_factor_docs = PromptsProvider.read_markdown_file(
            os.path.join(
                os.path.dirname(__file__), "../../../../seekgain_server/services/llm/factor_readme.md"
            )
        )
        shifted_seekgain_factor_docs = PromptsProvider.shift_markdown_headings(
            seekgain_factor_docs, shift=shift_level
        )
        return shifted_seekgain_factor_docs

    @staticmethod
    def get_factor_engine_doc():
        return PromptsProvider.join(
            "## Factor Engine(seekgain_factor) Documentation",
