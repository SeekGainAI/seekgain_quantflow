...
from seekgain_server.models.general_code_assistant_response import (
    GeneralCodeAssistantResponse, 
    GeneralCodeAssistantData, 
    GeneralCodeAssistantMessage
)
from seekgain_server.services.llm.agents.backtest_assistant import BacktestAssistant
from seekgain_server.services.llm.models.backtest_assistant_request import BacktestAssistantRequest
import json

logger = logging.getLogger(__name__)

async def backtest_assistant_nonstream_logic(
    uid: str,
    ...
    Returns:
        GeneralCodeAssistantResponse: 回测助手响应
    """
    try:
        # 实例化回测助手
        ...
        try:
            ...
            logger.error(f"Failed to parse backtest assistant response as JSON: {e}")
        ...
        logger.error(f"回测助手处理时发生错误: {str(e)}")
        raise Exception(f"回测助手处理失败: {str(e)}") 